...
TEST(TSet, plus_operator_applied_to_tset_of_three_positions)
{
const int size = 4;
TSet s1(size), s2(size), s3(size), expSet(size);
s1.InsElem(1);
s2.InsElem(2);
s3.InsElem(3);
expSet.InsElem(1);
...
TEST(TSet, throws_when_insert_negative_element)
{
	const int size = 10;
	TSet set(size);
...
TEST(TSet, triple_operator_plus)
{
const int size1 = 5, size2 = 7;
TSet set1(size1), set2(size2), set3(size1), expSet(size2), set4(size2);
// set1 = {1, 2, 4}
set1.InsElem(1);
set1.InsElem(2);
set1.InsElem(4);
